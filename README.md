...
I used Claude Code to build this. `AGENTS.md` and `CLAUDE.md` in this repo
aren't hand-written project conventions — they're auto-generated by `next
dev` itself (see `node_modules/next/dist/server/lib/generate-agent-files.js`),
pointing any agent at the version-specific docs in `node_modules/next/dist/docs/`
since Next 15's App Router APIs can differ from a model's training data.
`CLAUDE.md` just imports `AGENTS.md` (Claude Code's own convention for that).
They're committed as-is per the file's own comment: it gets re-added on every
...
directed the assistant conversationally: gave it the assignment text, had it
pick the data domain and file layout, and reviewed/tested each feature
(server rendering, the server action, the dynamic route, the 404 case) as it
was built rather than accepting it unread.
